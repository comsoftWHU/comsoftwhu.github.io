...
## 9. 按模块归纳：从起点到终点，具体“变了什么”

### 9.1 Compiler / Optimizing

起点更像：
- optimizing compiler 是主要编译形态
- 核心 IR/loop/container 类型集中在 `nodes.*`
- `RegisterSet` 只是 `Location` 附属概念
- visitor/pass 风格混杂
- RA/caller-save/pair/spill 模型耦合较重

终点更像：
- 新增 ARM64 fast compiler，形成 baseline 编译层
- `Graph` / `InstructionList` / `LoopInformation` / `RegisterSet` 成为独立基础设施
- `CRTPGraphVisitor` 成为 pass 统一骨架
- linear scan RA 更显式地处理 blocked/caller-save/hint/pair/spill
- codegen 与 RA 的接口边界更清晰

### 9.2 Runtime / GC / JIT

起点更像：
- GC/JIT 基础逻辑继续演进，但代际结构和 companion artifact 还不突出
- virtual thread 支撑还没有形成共同运行时模块

终点更像：
- CMC GC 已经进入 generational 模型
- runtime 显式承接 virtual thread/continuation 停车和 frame materialization
- JIT / root table / string intern 交互更保守更稳定
- 新增 SDC 这种 companion artifact，并进入 runtime/oat 语义

### 9.3 Verifier / HiddenAPI

起点更像：
- verifier 路径更历史积累式
- hiddenapi 主要还是 Java/reflection/managed caller 视角

终点更像：
- verifier 被重整成更平直的四段流程
- branch target / `move-exception` / `filled-new-array` / constructor return 等边界更明确
- hiddenapi 引入更完整的 domain 模型
- JNI/native caller 也被纳入 enforcement
- core-platform 与 platform 的边界被实质化

### 9.4 artd / Dexopt

起点更像：
- artd 是 dexopt 相关服务
- 但 staged metadata / SDC / pre-reboot 事务流还没形成完整体系

终点更像：
- artd 既是 dexopt 服务，也是产物状态服务
- pre-reboot dexopt 有 metadata、有 compatibility check、有 staged status、有 commit 流程
- SDC/SDM 等 companion artifact 被显式纳入生命周期管理
- dexopt 产物管理开始明显带有“事务性部署”的风格

---

## 10. 这段区间里，最值得你关注的“设计重心变化”

如果只看设计方法论，这两个提交之间最重要的变化有四条：
...
而是变成：
- 解释器 / nterp
- fast baseline compiler
- full optimizing compiler

这对后续所有“值模型改造”“寄存器模型改造”“spill slot 规则改造”都很重要，因为：
- fast compiler 和 optimizing compiler 的约束不一样
- baseline 路径更看重简单、直接、保守
- optimizing 路径更看重 IR 和 RA 模型一致性

### 10.2 `optimizing/` 开始去大文件化、去隐式耦合

最明显的信号就是：
- `nodes.*` 缩了
- `graph.*` / `instruction_list.*` / `loop_information.*` / `register_set.h` 出来了

这说明维护者已经意识到：
- 继续堆在 `nodes.h` / `locations.h` 里，扩展成本太高
- 后续想做更激进的编译器演进，必须先拆基础设施
...
### 10.4 ART 不再只是 runtime + compiler，也越来越像“系统服务组件”

artd / pre-reboot dexopt / staged metadata / SDC 的变化说明：
- ART 不再只是进程内 runtime
- 它也在承担系统升级、产物管理、服务接口、状态判断这些系统级职责

---

## 11. 对你当前阅读 `./art` 代码时的建议

如果你后续还要继续对 `./art` 做设计级分析，建议按下面顺序读：

1. `compiler/optimizing/fast_compiler.h`
2. `compiler/optimizing/fast_compiler_arm64.cc`
3. `compiler/optimizing/graph.h`
4. `compiler/optimizing/instruction_list.h`
5. `compiler/optimizing/loop_information.h`
6. `compiler/optimizing/register_set.h`
7. `compiler/optimizing/register_allocator_linear_scan.cc`
8. `runtime/gc/collector/mark_compact.h`
9. `runtime/virtual_thread_common.cc`
10. `runtime/hidden_api.cc`
11. `runtime/verifier/method_verifier.cc`
12. `artd/artd.cc`

原因很简单：
- 这些文件基本就把“新增 baseline 编译层、优化器拆分、runtime 新语义、服务化 dexopt”四条主线串起来了。
